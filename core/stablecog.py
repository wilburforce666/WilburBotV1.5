...
class StableCog(commands.Cog, name='Stable Diffusion', description='Create images from natural language.'):
    ...
    def __init__(self, bot, called_from_button=False):
        ...
        with open("config.json", "r") as cfg:
            self.filter_config = json.load(cfg)
        self.pipe = None
    ...
    async def dream_handler(self, ctx: discord.ApplicationContext, *,
        ...
        banned_pattern = re.compile(r'\b(?:loli|shota|child|children|underage|\d+\s*year\s*old)\b', re.IGNORECASE)
        if banned_pattern.search(prompt):

            embed = discord.Embed(
                title="Prompt Blocked",
                description="🚫 Your prompt contains **disallowed content** related to minors and has been rejected.",
                color=settings.global_var.embed_color
            )
            await ctx.channel.send(embed=embed)
            return  # Exit early without calling the generation API
    ...
    def dream(self, event_loop: queuehandler.GlobalQueue.event_loop, queue_object: queuehandler.DrawObject):
        user_id = str(queue_object.ctx.author.id)

        # Check if user is blocked from prior offenses
        if content_filter.is_user_blocked(user_id):
            embed = discord.Embed(
                title="Blocked",
                description="🚫 You are blocked from using this bot due to repeated content violations.",
                color=0xff0000
            )
            # Post a channel warning message
            try:
                event_loop.create_task(queue_object.ctx.channel.send(embed=embed))
            except Exception:
                pass
            return  # Stop the dream method here
